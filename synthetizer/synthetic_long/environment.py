...
class SynEnvironmentDeter(Environment):
    ...
    def __init__(self, classifier, encoder, scaler, dataset="data/synthetic_long/train.csv",
        ...
        self.prog_to_func = OrderedDict(sorted({'STOP': self._stop,
                                                ...
                                                'CHANGE_EXPENSES': self._change_expenses
                                                }.items()))
        ...
        self.prog_to_precondition = OrderedDict(sorted({'STOP': self._placeholder_stop,
                                                        ...
                                                        'CHANGE_SAVINGS': self._change_savings_p,
                                                        'CHANGE_PURPOSE': self._change_purpose_p,
                                                        'CHANGE_SCORE': self._change_score_p,
                                                        'CHANGE_EXPENSES': self._change_expenses_p,
        ...
        self.programs_library = OrderedDict(sorted({'STOP': {'level': -1, 'args': 'NONE'},
                                                    ...
                                                    'CHANGE_SAVINGS': {'level': 0, 'args': 'INT'},
                                                    'CHANGE_PURPOSE': {'level': 0, 'args': 'PURPOSE'},
                                                    'CHANGE_SCORE': {'level': 0, 'args': 'SCORE'},
                                                    'CHANGE_EXPENSES': {'level': 0, 'args': 'EXPENSES'},
        ...
        self.prog_to_cost = OrderedDict(sorted({'STOP': self._stop_cost,
                                                ...
                                                'CHANGE_SCORE': self._change_score_cost,
                                                'CHANGE_EXPENSES': self._change_expenses_cost
                                                }.items()))
        ...
        self.numerical_cols = ["age", "credit", "income", "savings", "expenses"]
        self.categorical_cols = ["education", "job", "house", "sex", "relationship", "country", "purpose", "credit_score"]
        self.boolean_cols = [
                                  "30 <= age < 40",
                                  "40 <= age < 50",
                                  "50 <= age < 60",
                                  ...
                                  "30000 <= savings < 50000",
                                  "50000 <= savings < 70000",
                                  "70000 <= savings < 100000",
                                  "100000 <= savings < 150000",
                                  "150000 <= savings < 200000",
                                  "savings >= 200000",
                                  "0 <= expenses < 500",
                                  "500 <= expenses < 1000",
                                  "1000 <= expenses < 2000",
    ...
    def _get_boolean_conditions(self, data):
        ...
        return [
            ...
            data[3] >= 200000,
            0 <= int(data[9]) < 10000,
            10000 <= int(data[9]) < 30000,
            30000 <= int(data[9]) < 50000,
            50000 <= int(data[9]) < 70000,
            70000 <= int(data[9]) < 100000,
            100000 <= int(data[9]) < 150000,
            150000 <= int(data[9]) < 200000,
            data[9] >= 200000,
            0 <= int(data[12]) < 500,
            500 <= int(data[12]) < 1000,
            1000 <= int(data[12]) < 2000,
            2000 <= int(data[12]) < 4000,
            data[12] >= 4000
        ]
    ...
    def _change_savings(self, arguments=None):
        self.memory[9] += arguments

    def _change_purpose(self, arguments=None):
        self.memory[10] = arguments

    def _change_score(self, arguments=None):
        self.memory[11] = arguments

    def _change_expenses(self, arguments=None):
    ...
    def _change_savings_p(self, arguments=None):
        return self.memory[9] > 0

    def _change_purpose_p(self, arguments=None):
        return self.arguments["PURPOSE"].index(arguments) < self.arguments["PURPOSE"].index(self.memory[10])

    def _change_score_p(self, arguments=None):
        return self.arguments["SCORE"].index(arguments) < self.arguments["SCORE"].index(self.memory[11])

    def _change_expenses_p(self, arguments=None):
        return self.memory[12]-arguments > 0
    ...
    def _change_purpose_cost(self, arguments=None):
        return self.arguments["PURPOSE"].index(arguments)+5

    def _change_score_cost(self, arguments=None):
        return self.arguments["SCORE"].index(arguments)+10

    def _change_expenses_cost(self, arguments=None):
        return self.arguments["EXPENSES"].index(arguments)+14 * self._rescale_by_job()
