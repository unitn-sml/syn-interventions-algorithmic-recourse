...
class ExampleEnvEncoder(nn.Module):
    '''
    ...
    def __init__(self, observation_dim, encoding_dim=20):
        super(ExampleEnvEncoder, self).__init__()
...
class ExampleEnv(Environment):
    ...
    def __init__(self, classifier, encoder, scaler, dataset="./data/example/example.csv",
                 sample_from_errors_prob=0.3, sample_env=True):

        # Dictionary which maps an atomic action to a function of the environment
        self.prog_to_func = OrderedDict(sorted({'STOP': self._stop,
                                                'EXAMPLE_ATOMIC_ACTION': self._example_atomic_action,
                                                }.items()))

        # Dictionary which maps atomic actions and intervene to a boolean precondition
        self.prog_to_precondition = OrderedDict(sorted({'STOP': self._placeholder_stop,
                                                        'EXAMPLE_ATOMIC_ACTION': self._example_atomic_action_p,
                                                        'EXAMPLE_INTERVENE_ACTION': self._example_intervene_action_p
                                                        }.items()))

        # Dictionary which maps the intervene action to the postcondition. The postcondition is needed to check if the
        # program worked correctly
        self.prog_to_postcondition = OrderedDict(sorted({
                                                        'EXAMPLE_INTERVENE_ACTION': self._example_intervene_action_postcontion
                                                        }.items()))

        # Dictionary which specify, for each atomic function, its level and which is the argument type it accepts.
        self.programs_library = OrderedDict(sorted({'STOP': {'level': -1, 'args': 'NONE'},
                                                    'EXAMPLE_ATOMIC_ACTION': {'level': 0, 'args': 'WORK'},
                                                    'EXAMPLE_INTERVENE_ACTION': {'level': 1, 'args': 'NONE'}
                                                    }.items()))

        # Dictionary which list, for each action, a LIST of the various possible values.
        ...
        # Dictionary which maps the various atomic actions to the cost of applying them to an environment
        self.prog_to_cost = OrderedDict(sorted({'STOP': self._stop_cost,
                                                'EXAMPLE_ATOMIC_ACTION': self._example_atomic_action_cost
                                                }.items()))

        # Dictionary that specify, for each argument type, a cost
        ...
        # Dictionaries which contains the categorical and numerical columns of the dataset. These are the columns
        # we want to use. The scaler and decoder must have been trained on these columns.
        ...
        # Name of the manually defined boolean columns to binarize the numerical values. In our case, the various
        # values are been divided into exclusive ranges.
        self.boolean_cols = [
                                  "age < 17",
                                  "19 <= age < 33",
                                  "33 <= age < 47",
        ...
        # Set up the system with various informations (this call is fixed)
        ...
        # Call parent constructor (this call is fixed)
