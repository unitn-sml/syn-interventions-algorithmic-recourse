...
def gower_matrix(
    data_x, data_y=None, weight=None, cat_features=None, norm=None, norm_ranges=None
):

    # function checks
    X = data_x.copy().astype(float)
    if data_y is None:
        Y = data_x.copy()
    else:
        Y = data_y.copy()
    ...
    num_features = np.array([i for i in range(Z.shape[1]) if i not in cat_features])
    Z_num = Z[:, num_features]

    num_cols = Z_num.shape[1]
    num_ranges = np.zeros(num_cols)
    num_max = np.zeros(num_cols)

    if norm is not None and norm_ranges is not None:
        num_max = norm
        num_ranges = norm_ranges
    else:
    ...
    weight_num = weight[num_features]
